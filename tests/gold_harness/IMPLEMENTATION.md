...
| *(differential queue — CLOSED 2026-09-24)* | **ALL 17 STEMS AUTHORED AND QUALIFIED** (the maintainer landed the full §18.7 set): the 14 solid stems (56 files) are IN THE CORPUS at 0/0 (244 files now) with their quads bit-identical and their OBSERVED outcomes recorded per row in §18.7 — the sweep spine is named (ExtrudeT: the draft_angle raw at bit 70, the SweepOptions order), the extrusion payload is the profile (ExtrudeR/P), the height lives in the direction (ExtrudeH), the loft height/radius slots are named (LoftH/R), the sweep frame direction entries are confirmed (PolysolidX/D), the revolve axis pair takes offset/tilt with world-X profile centers and normalized directions (RevolveO/T), 360° = plain 2π (RevolveF), and RevolveW is BIT-IDENTICAL to the original. The 3 M-stems (12 files) are QUARANTINED in `tests_quarantine/sh_history/` — valid specimens (gold clean, the ASSOC bodies typed with named parameters) but silver lacks the R2007+ surface-entity/action-body parsers (14 diffs/file), so they await the surface-parser row. **Decoder state (2026-09-24, commit 5c89b59): the spine naming and the extrusion profile circle are DECODED and pinned. The CLOSING SET (2026-09-24, the four §18.7 rows): `LoftC` (the raw-run namer — the landed original's prefix is wire-verified as [top cx][top cy][top z][top r]), `PolysolidW` (the sweep singles), `ExtrudeC` (the pre-CALL region), `LoftD` (the pi/2 pair / settings) — 16 files, corpus 244 -> 260. The ExtrudeP polyline header is AGENT work (the embedded-lwpolyline grammar is already in-repo); the revolve option shorts + flags have NO known authoring path (the REVOLVE command offers no draft/twist) — they close via the surface-parser row or stay marked like BREP.** | the blob-autopsy remainder (§18.6) | 2007/2010/2013/2018 each, landed |
...
| **P1 — CLOSING SET** | `LoftC_<v>` | `CIRCLE` center `3,4,0` radius `1.5` + `CIRCLE` center `3,4,7` radius `1.5`; `LOFT` both | the sections' world position + a non-1.0 radius (every landed loft section sits at the origin with r 1) | **THE loft raw-run namer.** Wire-verified reading of the landed original (2026-09-24, z-separated silhouette fits: bottom circle (0,0) r 1.0, top circle (2.006, 2.006) r 0.2996): its raws `[2.0, 2.0, 5.0, 0.3, π/2, π/2]` are **[top center.x][top center.y][top z][top radius]** — the "drag residue 2.0/2.0/0.3" was the dragged top section's geometry all along. PREDICTION: world reading -> raws `[3.0, 4.0, 7.0, 1.5, π/2, π/2]`; bottom-relative reading -> `[7.0, 1.5, π/2, π/2]` (the offset absent as two-bit shorts). Either outcome names the entire raw run |
| **P1 — CLOSING SET** | `PolysolidW_<v>` | `POLYSOLID` → `Height` 3 → `width` 7; path `0,0,0` → `12,0,0`, ENTER to end | the profile W/H swap vs `PolysolidX` (3/5 -> 7/3; the path identical) | **THE sweep-singles namer.** PolysolidX's unnamed singles are [2.0109, 3.0 (the width?), −19594.35]; with width 7 the width-tracking entry moves 3.0 -> 7.0, the corners read (±3.5, {0, 3}), and whatever still does not move is drag residue or transform data — the last opaque mass of the sweep frame blocks |
| **P1 — CLOSING SET** | `ExtrudeC_<v>` | `CIRCLE` center `2,3,0` radius `1`; `EXTRUDE` height `2` | the profile's world position (every landed extrusion circle sits at the origin) | **THE pre-CALL region test.** The 64-bit pre-CALL blob is byte-identical across all origin-centered circles (landed/H/R/T) and changes only with the profile SHAPE (ExtrudeP's polyline). If it moves with the offset it encodes the profile transform; if not it is shape-only. ALSO the first nonzero CALL-circle center — decodes (2, 3, 0): x raw 2.0, y raw 3.0, z short — and the CALL length grows 16 -> 144, a clean grammar anchor |
| **P2 — CLOSING SET** | `LoftD_<v>` | `CIRCLE` `0,0,0` r `1` + `CIRCLE` `0,0,5` r `1`; `LOFT` with the `Settings` option — set **Ruled**, and type `10` into any draft-angle field the dialog offers (record every field you see in the `.txt`) | the loft fit/settings (every landed loft uses the smooth default) | names the π/2 pair and the inter-value grammar: if the π/2 entries are draft/fit-related they move with the settings; if they stay they are geometry constants and the inter-value region is the settings container. The ASSOC body's named `Continuity`/`Bulge` pab-values (§18.6) are the semantic candidates |
